...
try:
	df = (sqlContext.read.parquet("s3n://big-data-analytics-scratch-prod/project_traveler_profile/affine/email_campaign/merged_shop_phase_date_format/{}/{}/{}".format(pos,locale_name,current_date)).filter("mer_status = 1"))
	log_df = log_df_update(sqlContext, 1,'traveler data imported',get_pst_date(),' ',str(df.count()),StartDate,' ','Orchestration.dbo.AlphaProcessDetailsLog')
except:
	log_df = log_df_update(sqlContext, 0,'failed',get_pst_date(),'current traveler data was not present',0,StartDate,' ','Orchestration.dbo.AlphaProcessDetailsLog')
...
df_final1.write.mode("overwrite").parquet("s3n://big-data-analytics-scratch-prod/project_traveler_profile/affine/email_campaign/segmentLookUp/"+pos+"/"+locale_name+"/"+data_environ)
print("-----Completed Segment Mapping process------")

LogID+=1
log_df = log_df_update(sqlContext,1,'Segment Mapping process completed.',get_pst_date(),' ',str(df_final.count()),segmentMappingStartDate,' ','Orchestration.dbo.AlphaProcessDetailsLog')
